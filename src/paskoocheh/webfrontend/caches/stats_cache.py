...
class StatsCacheManager(object):
    u"""
    ...
    stats.tasks.update_rating respectively
    """
    def __init__(self):
        self.register_signal_receivers()

    def get_normalized_platform_slug_name(self, slug_name):
        """
    ...
    def get_tool_id_platform_key(self, versiondownload):
        """
        ...
        return u'{tool_id}_{platform_slug_name}'.format(
            ...
            platform_slug_name=(
                self.get_normalized_platform_slug_name(versiondownload.platform_name)
            ),
        )

    def update_versiondownload_cache_values(self, sender, **kwargs):
        """
        ...
        for versiondownload in versiondownloads:
            versiondownload.id_platform_key = self.get_tool_id_platform_key(versiondownload)
    ...
    def update_versionrating_cache_values(self, sender, **kwargs):
        """
    ...
    def register_signal_receivers(self):
        """
        Register receivers for VersionDownload and VersionRating
        post_batch_update signals.
        ...
        post_batch_update.connect(
            self.update_versiondownload_cache_values,
            sender=VersionDownload,
            weak=False,
        )

        post_batch_update.connect(
            self.update_versionrating_cache_values,
