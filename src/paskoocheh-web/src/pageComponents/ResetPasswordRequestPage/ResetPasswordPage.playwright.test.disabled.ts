...
test("Renders as expected", async ({ page }) => {
  ...
  await expectPageMainHeadingAndMetadata({
    mainHeading: strings.ResetPasswordPage.title,
    metaDescription: strings.ResetPasswordPage.pageDescription,
    metaTitle: strings.ResetPasswordPage.title,
    page,
    path,
    strings,
  });
...
test("ResetPasswordPage form submission works", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: strings.shared.button.resetPassword })
    .click();
